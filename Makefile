...
all:
	...
	mkdir -p spend && cd spend && wget https://circuitscan-artifacts.s3.us-west-2.amazonaws.com/build/spend-nice-olive-raven/pkg.zip
	...
	cp spend/build/verify_circuit/groth16_vkey.json out/spend.zkey

	# echo "Downloading proof-of-burn-circuit build files..."
	# mkdir -p proof_of_burn && cd proof_of_burn && wget https://circuitscan-artifacts.s3.us-west-2.amazonaws.com/build/proofofburn-unconscious-sapphire-gorilla/pkg.zip
	...
	# cp proof_of_burn/build/verify_circuit/groth16_vkey.json out/proof_of_burn.zkey
